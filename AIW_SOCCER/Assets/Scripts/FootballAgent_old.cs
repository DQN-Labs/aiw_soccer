...
public static class Rewards_old
{
...
public class FootballAgent_old : Agent, ICubeEntity
{
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        {
            cubeEntity.BKick();
            AddReward(Rewards_old.kickPenalty);
        ...
        if (dashAction == 1 && cubeEntity.CanDash())
        {
            ...
            AddReward(Rewards_old.dashPenalty);
        }
        ...
        {
            AddReward(Rewards_old.ApproachBallReward);
        }
        ...
        if (ballVelocity.magnitude > 0.1f)
        {
            ...
            if (dot > 0.5f)
            {
                AddReward(Rewards_old.BallMovingTowardGoal * dot * (ballVelocity.magnitude / 4.5f));
        ...
        AddReward(Rewards_old.MovePenalty);
    }
    ...
    private void HandleGoalScored(object Sender, Net.OnGoalScoredEventArgs e)
    {
        ...
        NetID netID = e.TeamScored == Team.Kais ? NetID.AlbertNet : NetID.KaiNet;

        if (netID == netTarget.GetNetID())
        {
            AddReward(Rewards_old.GoalScoredReward);
            statsRecorder.Add($"Football/{name}/GoalsScored", 1, StatAggregationMethod.Sum);
        }
        else
        {
            AddReward(Rewards_old.GoalConcededPenalty);
        }
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.gameObject.CompareTag("Ball"))
        {
            ...
            {
                AddReward(Rewards_old.BallTouchReward);
