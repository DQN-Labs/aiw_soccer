...
public class SoccerAgent : Unity.MLAgents.Agent, ICubeEntity
{
    ...
    [SerializeField] private Net objectiveGoal;
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        sensor.AddObservation(objectiveGoal.transform.localPosition);
        sensor.AddObservation((ball.transform.localPosition - transform.localPosition).normalized);
        sensor.AddObservation((objectiveGoal.transform.localPosition - transform.localPosition).normalized);
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        if (rotateAction == 1)
            rigidBody.MoveRotation(rigidBody.rotation * Quaternion.Euler(0f, -rotationSpeed * Time.fixedDeltaTime, 0f));
        ...
        if (Vector3.Distance(ball.transform.localPosition, objectiveGoal.transform.localPosition) < 1.5f)
        {
            AddReward(1.0f);
            EndEpisode();
        }
